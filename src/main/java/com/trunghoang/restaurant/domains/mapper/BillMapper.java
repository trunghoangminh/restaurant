		...
		mapperFactory.classMap(Bill.class, BillDTO.class).exclude(CUSTOMER_ORDERS).byDefault()
				.customize(new CustomMapper<Bill, BillDTO>() {
					...
					public void mapAtoB(Bill a, BillDTO b, MappingContext context) {
						...
						// Map manually for customerOrder field to void infinity
						// loop(in A has B and in B has A)
						...
						for (CustomerOrder customerOrder : a.getCustomerOrders()) {
							...
							if (menu != null) {
								...
								List<String> additionalDetails = Arrays.asList(menu.getAdditionalDetails().split(
										SEPARATOR));
						...
						// When mapping done => nhuc' cai' dau` :'( choi gi` ma`
						// choi vong` tron`
					}
